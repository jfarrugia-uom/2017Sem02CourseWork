...
public:
	...
	double operator[](int p_nIndex) const { return Element[p_nIndex]; }
	double& operator[](int p_nIndex) { return Element[p_nIndex]; }

	inline void Set(double p_x, double p_y) {
		X = p_x; Y = p_y;
	}

	inline bool Equals(const Vector2 &p_vector, const double p_epsilon = 1e-5f) const 
	{
	...
	inline Vector2 operator*(double p_fValue) const {
	...
	inline Vector2& operator*=(double p_fValue) {
	...
	inline double MaxAbsComponent() const {
		return std::max(fabs(X), fabs(Y));
	}

	inline double MinAbsComponent() const
	{
	...
	inline double Dot(const Vector2 &p_vector) const {
		return Vector2::Dot(*this, p_vector);
	}

	inline double AbsDot(const Vector2 &p_vector) const {
		return Vector2::AbsDot(*this, p_vector);
	}

	static double Dot(const Vector2 &p_vector1, const Vector2 &p_vector2) {
		return p_vector1.X * p_vector2.X + p_vector1.Y * p_vector2.Y;
	}

	static double AbsDot(const Vector2 &p_vector1, const Vector2 &p_vector2) {
	...
	static double DistanceSquared(const Vector2 &p_point1, const Vector2 &p_point2) {
		return (p_point2 - p_point1).LengthSquared();
	}

	static double Distance(const Vector2 &p_point1, const Vector2 &p_point2) {
		return (p_point2 - p_point1).Length();
	}
};

inline Vector2 operator*(double p_fValue, const Vector2 &p_vector) {
